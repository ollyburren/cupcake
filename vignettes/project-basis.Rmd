...
```{r}
proj.dt <- project_sparse(beta=log(M$or),seb=M$seb,pids=M$pid)[,trait:='EGPA'][]
```
...
## project on original basis traits to characterise PCs
...
comb.dt <- rbind(bt.proj[,.(PC,delta,var.proj=0,trait)],proj.dt[,.(PC,delta,var.proj,trait)])
